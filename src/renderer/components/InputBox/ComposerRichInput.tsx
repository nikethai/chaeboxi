...
export type ComposerRichInputProps = {
  ...
  /** May preventDefault and return plain text for the editor to insert once. */
  onPaste?: (event: React.ClipboardEvent<HTMLDivElement>) => void | { insertPlainText?: string | null }
  placeholder?: string
  disabled?: boolean
  autoFocus?: boolean
  className?: string
...
const ComposerRichInput = forwardRef<ComposerRichInputHandle, ComposerRichInputProps>(function ComposerRichInput(
  {
    id,
    value,
    onChange,
  ...
  const insertPlainText = useCallback(
    (text: string) => {
      ...
      if (!ok) {
        // Fallback when execCommand is unavailable (some WebViews).
        const el = editorRef.current
        if (!el) return
  ...
  const handlePaste = useCallback(
    (event: React.ClipboardEvent<HTMLDivElement>) => {
      // Parent owns attachment policy (images/files/long-text). It may preventDefault
      // and optionally request plain-text insertion via the return value.
      const result = onPaste?.(event) as void | { insertPlainText?: string | null }
      if (event.defaultPrevented) {
        const text = result && typeof result === 'object' ? result.insertPlainText : null
        if (text) insertPlainText(text)
        return
      }
      const text = event.clipboardData.getData('text/plain')
      if (!text) {
        // Block native rich HTML paste into contenteditable when no plain text.
