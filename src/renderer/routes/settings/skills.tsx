    ...
    <SettingsPage wide>
      ...
      {isDesktop && (
        <SettingsSection
          ...
          description={t(
            'Shared with Claude Code, Codex, Cursor, and other agents. Project folders win over global ones when names collide.'
          )}
        >
          <SettingsCard>
            <div className="settings-card-fields">
              <Text size="xs" c="chatbox-tertiary">
                {agentSkillCount > 0 ? `${agentSkillCount} skills` : t('No agent skills scanned yet')}
              </Text>
              <Stack gap={4}>
                {(agentRoots.length
                  ? agentRoots
                  : AGENT_SKILL_ROOTS.map((r) => ({ path: r.path, origin: r.origin, exists: false }))
                ).map((root) => (
                  <Text
                    key={root.path}
                    size="xs"
      ...
      <SettingsSection title={t('Catalog')}>
        ...
        <Modal
          ...
          title={editing?.id ? t('Edit skill') : t('New skill')}
          size="lg"
        >
          <Stack gap="md">
            <TextInput
